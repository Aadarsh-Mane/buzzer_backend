...
io.on("connection", (socket) => {
  ...
  socket.on("media-status", async ({ interviewId, userId, audioEnabled, videoEnabled }) => {
    try {
      console.log(colors.cyan(`🎥 Media status update from ${userId}: audio=${audioEnabled}, video=${videoEnabled}`));
  ...
  socket.on("user-media-ready", async ({ interviewId, userId, role }) => {
    try {
      console.log(colors.green(`📹 User media ready: ${userId} (${role}) in interview ${interviewId}`));
      ...
      console.log(colors.cyan(`📡 Broadcasted user-media-ready for ${userId} to interview ${interviewId}`));
    } catch (err) {
