...
io.on("connection", (socket) => {
  ...
  socket.on("webrtc-offer", async ({ interviewId, offer, to }) => {
    try {
      console.log(
        colors.blue(
          `📤 WebRTC offer from ${socketUserId} to ${to} in interview ${interviewId}`
        )
      );
      ...
      const targetSocket = Array.from(io.sockets.sockets.values()).find(
        (s) => s.rooms.has(interviewId) && s.socketUserId === to
      );
  ...
  socket.on("webrtc-answer", async ({ interviewId, answer, to }) => {
    try {
      console.log(
        colors.blue(
          `📥 WebRTC answer from ${socketUserId} to ${to} in interview ${interviewId}`
        )
      );
  ...
  socket.on("webrtc-ice-candidate", async ({ interviewId, candidate, to }) => {
    try {
      console.log(
        colors.blue(
          `🧊 ICE candidate from ${socketUserId} to ${to} in interview ${interviewId}`
        )
      );
